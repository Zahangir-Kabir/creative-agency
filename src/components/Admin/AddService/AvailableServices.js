...
const AvailableServices = () => {
    ...
    useEffect(() => {
        fetch(`https://nameless-journey-31072.herokuapp.com/serviceItem`)
            .then(res => res.json())
            .then(data => {
                setServices(data)
            })
    ...
    const handleDelete = (id) => {
        console.log(id)
        fetch(`https://nameless-journey-31072.herokuapp.com/deleteItem/${id}`, {
            method: 'DELETE',
        })
            .then(res => res.json())
            .then(data => {
        ...
        <div>
            ...
            <div className="d-flex flex-wrap p-2" style={{ height: '450px', overflow: 'auto', backgroundColor: '#f4f7fc' }}>
                <table className="table table-hover p-1 bg-white " >
                    <thead style={{ fontWeight: 'blod' }}>
                        <tr className="table-primary" style={{ fontWeight: 'blod' }}>
                            <th scope="col">#</th>
                    ...
                    <tbody style={{ height: "600px", overflow: 'hidden' }}>
                        {
                            services.map((each, index) =>
                                <tr style={{ fontWeight: 'blod' }}>
                                    <td>{index + 1}</td>
